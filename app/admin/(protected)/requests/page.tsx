    ...
    <div>
      ...
      <div className="grid grid-cols-3 gap-4 mb-8">
        {requestStatuses.map((s) => (
          <div key={s} className="bg-white rounded-2xl border border-slate-200 p-5 text-center">
            <p className="text-2xl font-bold">{countsByStatus[s] ?? 0}</p>
            <p className="text-xs text-slate-500 uppercase tracking-wide mt-1">{s.replace("_", " ")}</p>
          </div>
        ))}
      </div>
          ...
          <tbody>
            {requests.map((r) => (
              <tr key={r.id} className="border-b border-slate-100 last:border-0">
                ...
                <td className="px-4 py-3 text-slate-600">{r.assignedAdmin?.name ?? "—"}</td>
                <td className="px-4 py-3 text-slate-600">{r.createdAt.toLocaleDateString()}</td>
                <td className="px-4 py-3">
                  <Link href={`/admin/requests/${r.id}`} className="text-emerald-700 hover:underline">
                    Open
                  </Link>
                </td>
              </tr>
